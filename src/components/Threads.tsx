...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }: ThreadsProps) => {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, dpr: 1 }); // dpr:1 = big perf win
    ...
    let lastFrame = 0;
    const frameDuration = 1000 / 30; // cap at ~30fps for background effect

    function update(t: number) {
      animationFrameId.current = requestAnimationFrame(update);

      // Skip if not visible or throttled
